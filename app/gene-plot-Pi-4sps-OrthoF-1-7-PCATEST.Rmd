...
"))
#helper function to get expression matrix based on transformation
get_expression_matrix <- function(species_code, transform_type, species_data = NULL) {
  ...
  if (transform_type == "rlog") {
    #get rlog matrix
    rlog_name <- if(species_code == "cg") "rlog" else paste0(species_code, "_rlog")
    expr_matrix <- species_data[[rlog_name]]
  } else {
    #get lcpm matrix (default)
    lcpm_name <- if(species_code == "cg") "lcpm" else paste0(species_code, "_lcpm")
    expr_matrix <- species_data[[lcpm_name]]
  }
  
  #DEBUG: print matrix info
  if (!is.null(expr_matrix)) {
    cat("\n=== get_expression_matrix DEBUG ===\n")
    cat("Species:", species_code, "\n")
    cat("Transform:", transform_type, "\n")
    cat("Matrix name:", if(transform_type == "rlog") rlog_name else lcpm_name, "\n")
    cat("Rows:", nrow(expr_matrix), "Cols:", ncol(expr_matrix), "\n")
    cat("First 5 rownames:", paste(head(rownames(expr_matrix), 5), collapse=", "), "\n")
    cat("Sample rowname for PHO84:", rownames(expr_matrix)[grep("PHO84|CAGL0B02475", rownames(expr_matrix), ignore.case=TRUE)][1], "\n")
  } else {
...
process_gene_group_data <- function(gene_groups, species_data, all_species_data, config, species_code) {
  ...
  # combines all data
  do.call(rbind, Filter(Negate(is.null), expression_data))
}
extract_orthology_for_genes <- function(gene_list, all_species_data = NULL, config = NULL) {
  ...
  cat("DEBUG: Searching for genes:", paste(gene_list, collapse=", "), "\n")
  ...
  for (gene in gene_list) {
    ...
    if (!is.null(query_result) && !is.null(query_result$genes_by_species)) {
      ...
      cat("DEBUG: Mapped", gene, "to orthogroup", query_result$orthogroup, "with",
          sum(sapply(gene_entry[names(config)], length)), "total genes\n")
    }
    
    if (!gene_found) {
      cat("WARNING: Gene", gene, "not found in any orthogroup\n")
    }
  }
  
  cat("DEBUG: Total input genes mapped:", length(ortholog_gene_map), "\n")
...
process_pathway_comparison <- function(pathway_definitions, species_data_list, config, all_species_data) {
  ...
  for (pathway_name in names(pathway_definitions)) {
    ...
    if (is.null(gene_mapping) || length(gene_mapping) == 0) {
      cat("WARNING: No orthologs found for pathway:", pathway_name, "\n")
      next
    }
    
    cat("DEBUG: Mapped", length(gene_mapping), "genes for pathway:", pathway_name, "\n")
    ...
    if (is.null(pathway_expr) || nrow(pathway_expr) == 0) {
      cat("WARNING: No expression data for pathway:", pathway_name, "\n")
      next
    }
  ...
  combined_data <- do.call(rbind, pathway_results)
...
calculate_pathway_foldchange <- function(pathway_data) {
  ...
  fc_data <- pathway_data %>%
    ...
    arrange(Timepoint) %>%
    mutate(
      BaselineExpr = MeanExpression[Timepoint == "0min"],
...
create_pathway_heatmap <- function(pathway_data, value_type = "foldchange", 
  ...
  cat("DEBUG: Found", n_pathways, "pathways:", paste(all_pathways, collapse=", "), "\n")
  cat("DEBUG: Found", n_species, "species:", paste(species_list, collapse=", "), "\n")
  
  #optional clustering of pathways (globally across all species)
  pathway_order <- all_pathways
  if (cluster_pathways && n_pathways > 1) {
    #create average expression matrix for clustering
    avg_matrix <- heatmap_data %>%
      group_by(Pathway, Timepoint) %>%
      summarise(AvgValue = mean(get(value_col), na.rm = TRUE), .groups = 'drop') %>%
      pivot_wider(names_from = Timepoint, values_from = AvgValue) %>%
      column_to_rownames("Pathway") %>%
      as.matrix()
    
    if (nrow(avg_matrix) > 1) {
  ...
  for (i in 1:n_species) {
    ...
    #create matrix: pathways x timepoints with consistent ordering
    heat_matrix <- sp_data %>%
      select(Pathway, Timepoint, all_of(value_col)) %>%
      pivot_wider(names_from = Timepoint, values_from = all_of(value_col))
    
    #ensure all pathways are present (fill with NA if missing)
    for (pw in pathway_order) {
      if (!pw %in% heat_matrix$Pathway) {
        new_row <- data.frame(Pathway = pw, stringsAsFactors = FALSE)
        for (tp in TIME_POINTS) {
          new_row[[tp]] <- NA
        }
        heat_matrix <- rbind(heat_matrix, new_row)
      }
    }
    
    #order by pathway_order
    ...
    heat_matrix <- heat_matrix[pathway_order, ]
    ...
    p <- plot_ly(
      ...
      text = matrix(
        paste(
          "Species:", sp,
          "<br>Pathway:", rep(rownames(heat_matrix), each = ncol(heat_matrix)),
          "<br>Timepoint:", rep(colnames(heat_matrix), times = nrow(heat_matrix)),
          "<br>Value:", round(as.vector(t(heat_matrix)), 2)
        ),
    ...
    p <- p %>% layout(
      ...
      yaxis = list(
        ...
        showticklabels = (i == 1 || i == 3)
      ),
      plot_bgcolor = plot_bg_color,
      paper_bgcolor = plot_bg_color,
      font = list(color = text_color),
      margin = list(l = if(i == 1 || i == 3) 120 else 20, r = 20, t = 50, b = 80)
    )
    
    subplot_list[[i]] <- p
    
    #add species label annotation
    annotations[[i]] <- list(
      text = paste0("<b>", sp, "</b>"),
      xref = "paper",
      yref = "paper",
      x = if(i == 1 || i == 3) 0.24 else 0.74,
      y = if(i <= 2) 0.98 else 0.48,
      xanchor = "center",
      yanchor = "top",
      showarrow = FALSE,
...
create_multi_species_pca <- function(get_species_data, is_dark_mode = FALSE, aggregation_method = "eigengene", species_config = DEFAULT_SPECIES_CONFIG, all_species_data_obj = NULL, transform_type = "lcpm") {
  ...
  print(paste("Debug: Starting HOG-based species PCA with", method_label))
  
  # Get configuration and data (with extensible defaults)
  ...
  if (aggregation_method == "single_only") {
    ...
    print("Debug: Using aggregation method for multi-copy HOGs")
    
    common_hogs <- names(hog_gene_map)[sapply(hog_gene_map, function(hog) {
      length(hog) >= 2
    })]
    
    print(paste("Found", length(common_hogs), "HOGs with at least 2 species"))
    
    #get statistics on multi-copy status
    single_copy_count <- sum(sapply(hog_gene_map[common_hogs], function(hog) {
      all(sapply(hog, length) == 1)
    }))
    multi_copy_count <- length(common_hogs) - single_copy_count
    
    print(paste("  -", single_copy_count, "are single-copy HOGs"))
    print(paste("  -", multi_copy_count, "are multi-copy HOGs"))
  }
  ...
  for (species_id in names(config)) {
    ...
    if (!is.null(sp_data)) {
      ...
      lcpm_matrix <- get_expression_matrix(species_id, transform_type, sp_data)
  ...
  #track aggregation statistics BEFORE filling matrix
  print(paste("Debug: Calculating HOG composition statistics"))
  ...
  print("Debug: HOG composition by species:")
  for (sp_code in names(aggregation_stats)) {
    stats <- aggregation_stats[[sp_code]]
    print(paste("  ", sp_code, ": ", stats$total_hogs, " total HOGs (", 
                stats$single_copy, " single-copy, ", 
                stats$multi_copy, " multi-copy)", sep = ""))
  }
  
  #pre-aggregate HOGs once per species 
  print("Debug: Pre-aggregating HOGs for each species")
  ...
  for (species_id in names(config)) {
    ...
    #get expression matrix based on transformation setting
    lcpm_matrix <- get_expression_matrix(species_id, transform_type, sp_data)
    
    if (is.null(lcpm_matrix)) next
    
    print(paste("  pre-aggregating", species_id, "-", nrow(lcpm_matrix), "genes,", 
                ncol(lcpm_matrix), "samples"))
    
    #build species-specific HOG map
    species_hog_map <- lapply(hog_gene_map[common_hogs], function(hog) {
      if (!is.null(hog[[species_id]])) {
        return(hog[[species_id]])
      } else {
        return(NULL)
      }
    })
    ...
    #get expression matrix for this species
    expr_matrix <- get_expression_matrix(species_id, transform_type, species_data_list[[species_id]])
    
    #pre-aggregate all HOGs for this species
    species_hog_expr[[species_id]] <- preaggregate_species_hogs(
      lcpm_matrix = expr_matrix,
  ...
  #now fill matrix using pre-aggregated values
  print("Debug: Filling matrix from pre-aggregated HOG expression")
  ...
  for (i in 1:nrow(sample_metadata)) {
    ...
    lcpm_matrix <- get_expression_matrix(species_code, transform_type, sp_data)
    ...
    for (j in 1:length(common_hogs)) {
      ...
      if (!is.null(species_hog_expr[[species_code]][[hog]])) {
        #extract value for this sample from pre-aggregated vector
  ...
  #print match and aggregation statistics
  print("Debug: Match statistics by species:")
  for (sp in names(matches_by_species)) {
    print(paste(sp, ":", matches_by_species[[sp]], "HOG values filled"))
    if (aggregation_method != "single_only") {
      print(paste("  -", aggregation_stats[[sp]]$single_copy, "single-copy HOGs"))
      print(paste("  -", aggregation_stats[[sp]]$multi_copy, "multi-copy HOGs (aggregated)"))
    }
  }
...
extract_ortholog_expression <- function(gene_mapping, species_data_list, config = NULL, transform_type = "lcpm") {
  ...
  for (i in 1:length(gene_mapping)) {
    ...
    combined_expr <- do.call(rbind, expression_by_species)
    
    if (!is.null(combined_expr) && nrow(combined_expr) > 0) {
  ...
  if (length(expression_data) > 0) {
    all_expression <- do.call(rbind, expression_data)
...
create_orthogroup_selection_ui_enhanced <- function(query_result, prefix) {
...
# Add this new function to create a formatted orthogroup table
create_orthogroup_details_table <- function(query_result) {
  if (is.null(query_result) || is.null(query_result$genes_by_species)) {
    return(NULL)
  }
  ...
  for (sp in names(query_result$genes_by_species)) {
    ...
    if (nrow(genes_df) > 0) {
      ...
      gene_info <- genes_df[, c("Species", "gene_id", "gene_name", "display")]
      all_genes <- rbind(all_genes, gene_info)
    }
  }
...
#new ggtree module for phylo tree (react fixed)
      ...
      tags$li(
        ...
        tags$ul(
          lapply(names(gene_counts)[gene_counts > 0], function(sp) {
            ...
            config <- if(!is.null(config)) config else current_species_config()
...
create_ortholog_mapping_table <- function(gene_mapping, config) {
  ...
  mapping_df <- do.call(rbind, rows_list)
  return(mapping_df)
}
...
process_multi_species_gene_set <- function(gene_mapping, species_data_list, config) {
  ...
  if (length(all_expression_data) > 0) {
    return(do.call(rbind, all_expression_data))
  } else {
    return(NULL)
  }
}
  ...
  tryCatch({
    ...
    if (!is.null(state$gene_list)) {
      tryCatch({
        ...
        message("could not restore gene_list: ", e$message)
      })
    }
    ...
    if (!is.null(state$enable_ortholog_analysis)) {
      tryCatch({
        ...
        message("could not restore enable_ortholog_analysis: ", e$message)
      })
    }
    ...
    if (!is.null(state$group_analysis_species)) {
      tryCatch({
        ...
        message("could not restore group_analysis_species: ", e$message)
      })
    }
    ...
    if (!is.null(state$group_viz_type)) {
      tryCatch({
        ...
        message("could not restore group_viz_type: ", e$message)
      })
    }
    ...
    if (!is.null(state$distance_method)) {
      tryCatch({
        ...
        message("could not restore distance_method: ", e$message)
      })
    }
    ...
    if (!is.null(state$data_transform)) {
      tryCatch({
        ...
        message("could not restore data_transform: ", e$message)
      })
    }
    ...
    if (!is.null(state$aggregation_level)) {
      tryCatch({
        ...
        message("could not restore aggregation_level: ", e$message)
      })
    }
    ...
    if (!is.null(state$ortholog_selected)) {
      tryCatch({
        ...
        message("could not restore ortholog_selected: ", e$message)
      })
    }
    ...
    if (!is.null(state$current_tab)) {
      tryCatch({
        ...
        message("could not restore current_tab: ", e$message)
      })
    }
  }, error = function(e) {
    message("error in restore_session_state: ", e$message)
  })
}
...
server <- function(input, output, session) {
  ...
  observeEvent(input$restore_session, {
    ...
    message("restore session triggered")
    
    state <- input$restore_session$state
    saved_at <- input$restore_session$saved_at
    time_ago <- format_time_ago(saved_at)
    
    message(paste("restoring state from", time_ago))
    message(paste("state contents:", paste(names(state), collapse = ", ")))
    ...
    shinyjs::delay(500, {
      ...
      message("state restored, showing notification")
  ...
  observe({
    ...
    isolate({
      ...
      message("session initialized, auto-save enabled")
    })
  }) %>% bindEvent(TRUE, once = TRUE)
  
  #auto-save session after changes using observeEvent pattern
  observe({
    #only run after initialization
    req(session_initialized())
    
    #dependencies that trigger save
    list(
      reactiveValuesToList(selected_genes),
      reactiveValuesToList(query_results),
      reactiveValuesToList(combined_selections),
      global_query_state$current_query,
      input$gene_list,
      input$enable_ortholog_analysis,
      input$nav,
      input$group_analysis_species,
      input$group_viz_type,
      input$distance_method,
      input$data_transform,
      input$aggregation_level
    )
    
    #isolate the actual save operation
    isolate({
      #skip if critical values are null (not yet loaded)
      if (is.null(input$nav)) return()
  ...
  observeEvent(input$run_pca, {
    ...
    if (input$pca_type == "multi") {
      cat("Debug: Aggregation method:", input$hog_aggregation_method, "\n")
    }
    ...
    if (!exists(species_id, envir = species_data_cache)) {
      ...
      if (species_id %in% names(current_data)) {
        ...
        if (!is.null(current_data[[species_id]]$lcpm) || !is.null(current_data[[species_id]]$rlog)) {
          #copy all data directly from current_data
          data <- current_data[[species_id]]
          
          #ensure species_name is set
        ...
        #verify we found at least one expression matrix
  ...
  observeEvent(input$enable_ortholog_analysis, {
    if (input$enable_ortholog_analysis) {
      ...
      if (is.null(gene_list) || length(gene_list) == 0) {
        showNotification("Please enter a gene list first", type = "warning", duration = 3)
  ...
  parse_pathway_definitions <- reactive({
    if (input$enable_pathway_comparison) {
      ...
      pathway_text <- if (!is.null(input$pathway_file$datapath)) {
        ...
        strsplit(input$pathway_definitions, "[\n\r]+")[[1]]
      } else {
        return(NULL)
      }
  ...
  observeEvent(input$analyze_gene_groups, {
    ...
    tryCatch({
      ...
      if (input$enable_pathway_comparison) {
        ...
        #ensure ortholog analysis is enabled for pathway comparison
        if (!input$enable_ortholog_analysis || !ortholog_state$mapped) {
          showNotification("Pathway comparison requires cross-species ortholog analysis. Enabling now...", 
                           type = "message", duration = 3)
          
          #enable ortholog analysis
          updateCheckboxInput(session, "enable_ortholog_analysis", value = TRUE)
          
          #wait briefly for ortholog mapping
          Sys.sleep(0.5)
        }
        ...
        output$gene_group_table <- renderDT({
          ...
          datatable(
            ...
            formatRound(columns = which(sapply(summary_table, is.numeric)), digits = 2)
        })
