...
load(file.path("..", "data", "RData_perSpecies_HOG_clean_08292025_gene_lookup_tree.RData"))
...
# Modified gene search function for basic compatibility
  ...
  for (i in 1:length(gene_mapping)) {
    ...
    for (species_code in names(species_data_list)) {
      ...
      if (!is.null(gene_map[[species_code]])) {
        ...
        # Get LCPM data - try direct access first, then prefixed
        lcpm_data <- species_data_list[[species_code]]$lcpm
        if (is.null(lcpm_data)) {
          lcpm_data <- species_data_list[[species_code]][[paste0(species_code, "_lcpm")]]
        }
        ...
        if (species_code == "kl" && !is.null(gene_id)) {
          ...
          if (gene_id_with_underscore %in% rownames(lcpm_data)) {
            gene_id <- gene_id_with_underscore
          }
        }
        
        if (gene_id %in% rownames(lcpm_data)) {
          ...
          # CRITICAL FIX: Extract expression values in the order of sample_info$Sample
          ...
          if ("Sample" %in% colnames(sample_info)) {
            ...
            expression_values <- as.numeric(lcpm_data[gene_id, sample_info$Sample])
          } else {
            # Fallback if Sample column doesn't exist - assume order matches
            expression_values <- as.numeric(lcpm_data[gene_id, ])
          }
...
capture_session_state <- function(selected_genes, query_results, global_query_state, combined_selections, input) {
  list(
    version = "1.0",
...
server <- function(input, output, session) {
  ...
  observeEvent(input$restore_session, {
    ...
    restore_session_state(state, selected_genes, query_results, global_query_state, combined_selections, session)
  ...
  observe({
    #force reactivity by converting to list
    ...
    #also watch input fields
    input$gene_list
    input$enable_ortholog_analysis
    input$nav
    
    #delay execution to avoid saving empty initial state
    invalidateLater(100)
