...
load("RData_perSpecies_HOG_clean_08292025_gene_lookup_tree.RData")
...
extract_orthology_for_genes <- function(gene_list, all_species_data = NULL, config = NULL) {
  ...
  # Get gene lookup table directly - more efficient
  lookup_table <- all_species_data$gene_lookup
  ...
  for (gene in gene_list) {
    ...
    # Search in gene lookup table directly
    matches <- lookup_table[toupper(gene_id) == gene_upper | toupper(gene_name) == gene_upper]
    
    cat("DEBUG: Gene", gene, "found", nrow(matches), "matches in lookup table\n")
    
    if (nrow(matches) > 0) {
      # Get the HOG for this gene
      first_match <- matches[1,]
      hog_id <- first_match$hog_id
      
      cat("DEBUG: Gene", gene, "belongs to HOG:", hog_id, "\n")
      
      if (!is.na(hog_id) && hog_id != "") {
        # Get all genes in this HOG
        hog_members <- lookup_table[hog_id == hog_id]
        
        # Build entry for this specific gene
        gene_entry <- list(original = gene)
        
        for (sp_code in names(config)) {
          sp_genes <- hog_members[species == sp_code]
          if (nrow(sp_genes) > 0) {
            # Find the best match for this species
            # First try exact match to the query gene
            exact_match <- sp_genes[toupper(gene_id) == gene_upper | toupper(gene_name) == gene_upper]
            if (nrow(exact_match) > 0) {
              gene_entry[[sp_code]] <- exact_match$gene_id[1]
            } else {
              # Use first gene from this species in the HOG
              gene_entry[[sp_code]] <- sp_genes$gene_id[1]
            }
          } else {
        ...
        cat("DEBUG: Mapped", gene, "to:", 
            paste(sapply(names(config), function(s) paste0(s, "=", gene_entry[[s]])), collapse=", "), "\n")
      } else {
        cat("DEBUG: No HOG found for gene", gene, "\n")
      }
    }
  ...
  cat("DEBUG: Total genes mapped:", length(ortholog_gene_map), "\n")
