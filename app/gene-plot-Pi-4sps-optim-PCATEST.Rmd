...
# Set to TRUE only for debugging - disable in production for better performance
DEBUG_MODE <- FALSE
...
# ============ MEMOIZATION CACHE ============
# Create a cache environment for expensive operations
.performance_cache <- new.env(hash = TRUE)

# Helper function to get/set cached values
cache_get_or_compute <- function(cache_key, compute_fn, ...) {
  if (exists(cache_key, envir = .performance_cache)) {
    return(get(cache_key, envir = .performance_cache))
  }
  
  result <- compute_fn(...)
  assign(cache_key, result, envir = .performance_cache)
  return(result)
}
...
# Pre-process gene lookup table once at startup for optimal performance
if (!is.null(all_species_data$gene_lookup)) {
  ...
  # Pre-compute uppercase versions for faster case-insensitive matching
...
"))
#helper function to get expression matrix based on transformation - optimized
get_expression_matrix <- function(species_code, transform_type, species_data = NULL) {
  ...
  # Pre-compute matrix name based on naming convention
  ...
  # Debug output only when DEBUG_MODE is TRUE
  if (DEBUG_MODE) {
...
#function to query genes using the lookup table - optimized with pre-computed uppercase
query_gene_lookup <- function(query, species_filter = NULL, current_data = NULL) {
  ...
  # Use pre-computed uppercase columns if available for faster matching
  if ("gene_id_upper" %in% names(lookup_table)) {
    matches <- lookup_table[gene_id_upper == query | gene_name_upper == query, ]
  } else {
    #fallback to toupper() if pre-computed columns don't exist
...
#modified query function for HOG + Gene Lookup - optimized
...
create_pca_plot <- function(expression_matrix, sample_info, is_dark_mode = FALSE) {
  ...
  # data filtering step - use vectorized rowVars for better performance
  ...
  # Pre-compute plot styling once (avoid repeated conditionals)
  plot_bg_color <- if(is_dark_mode) "#2c3034" else "white"
  text_color <- if(is_dark_mode) "white" else "black"
  grid_color <- if(is_dark_mode) "gray30" else "gray90"
...
create_multi_species_pca <- function(get_species_data, is_dark_mode = FALSE, aggregation_method = "eigengene", species_config = DEFAULT_SPECIES_CONFIG, all_species_data_obj = NULL, transform_type = "lcpm", debug = FALSE) {
  ...
  # Get data for all species and CACHE expression matrices upfront
  # This avoids repeated calls to get_expression_matrix throughout the function
  species_data_list <- list()
  expression_matrix_cache <- list()  # Cache for expression matrices
  sample_info_cache <- list()        # Cache for sample info
  ...
  for (species_id in names(config)) {
    ...
    if (!is.null(sp_data)) {
      # Cache expression matrix once per species
  ...
  #create sample metadata dynamically using cached data
  ...
  #pre-aggregate HOGs once per species using cached matrices
  ...
  for (species_id in names(config)) {
    ...
    #pre-aggregate all HOGs for this species (using cached matrix)
  ...
  # Pre-build species code lookup for faster access
  ...
  #now fill matrix using pre-aggregated values (optimized)
  for (i in 1:nrow(sample_metadata)) {
    sample_name <- sample_metadata$Sample[i]
    species <- sample_metadata$Species[i]
    
    #find species code using pre-built lookup (faster than loop)
    ...
    #fill HOG values from pre-aggregated data (vectorized where possible)
  ...
  # Impute remaining NAs with column means - vectorized approach
  ...
  # Vectorized imputation using column means
...
# Ridgeline plot functions - optimized with list collection
create_ridgeline_plot <- function(species_data_list, is_dark_mode = FALSE) {
  # Prepare data using list collection (much faster than repeated rbind)
  ...
  for (species_id in names(species_data_list)) {
    ...
    # For each sample, extract expression values and collect in list
    for (i in 1:nrow(sample_info)) {
      ...
      # Store in list (faster than rbind in loop)
  ...
  # Combine all at once using rbindlist (highly optimized)
...
# Function to create gene count threshold plots - optimized with list collection
...
# Add this new function to create a formatted orthogroup table - optimized
create_orthogroup_details_table <- function(query_result) {
  if (is.null(query_result) || is.null(query_result$genes_by_species)) {
    return(NULL)
  }
  
  # Collect all gene information using list collection (faster than rbind)
...
#new ggtree module for phylo tree (react fixed) - optimized
create_phylo_tree_plot <- function(tree, orthogroup_genes, orthogroup_id, is_dark_mode = FALSE, current_data = NULL, species_colors = NULL, species_config = NULL) {
  ...
  # Create a mapping of gene IDs using list collection (faster than rbind)
...
server <- function(input, output, session) {
  ...
  manage_combined_containers <- function(config) {
    ...
    # Add containers for current configuration (no blocking sleep needed with immediate = TRUE)
    for (sp_id in names(config)) {
  ...
  # Clear performance cache when data source changes
  observeEvent(data_source(), {
    clear_performance_cache()
    debug_print("Performance cache cleared due to data source change")
  }, ignoreInit = TRUE)
  ...
  observeEvent(input$run_pca, {
    ...
    # Debug output (only when DEBUG_MODE is TRUE)
    debug_cat("Debug: PCA type selected:", input$pca_type, "\n")
    if (input$pca_type == "multi") {
      debug_cat("Debug: Aggregation method:", input$hog_aggregation_method, "\n")
    }
    
    tryCatch({
      if (input$pca_type == "single") {
        debug_cat("Debug: Entering single-species PCA branch\n")
        ...
        debug_cat("Debug: Entering multi-species PCA branch\n")
      ...
      debug_cat("Debug: Error occurred:", e$message, "\n")
      showNotification(
  ...
  observeEvent(input$validate_uploads, {
    ...
    for (species_id in names(species)) {
      ...
      if (!is.null(expr_data) && DEBUG_MODE) {
        ...
        debug_print(paste("First few row names (gene IDs):", paste(head(rownames(expr_data), 3), collapse=", ")))
        debug_print(paste("Dimensions:", nrow(expr_data), "x", ncol(expr_data)))
        debug_print(paste("All columns numeric?", all(sapply(expr_data, is.numeric))))
      }
