...
derive_species_colors <- function(species_list, palette_name, custom_overrides = NULL) {
...
derive_species_shapes <- function(species_list, custom_overrides = NULL) {
  n <- length(species_list)
...
generate_default_settings <- function(species_config) {
  ...
  list(
    ...
    species_colors = derive_species_colors(species_list, "Dark2"),
    species_shapes = derive_species_shapes(species_list),
...
server <- function(input, output, session) {
  ...
  #collect custom species shapes from pickers
  observe({
    config <- current_species_config()
    species_list <- sapply(config, function(x) x$short)
    custom <- list()
    ...
    if (length(custom) > 0) {
      plot_settings$species_shapes <- custom
    }
  })
  ...
  observeEvent(input$combined_plot_button, {
    ...
    if (nrow(plot_data) > 0) {
      output$combined_gene_plot <- renderPlotly({
        ...
        if (isTRUE(input$normalize_to_baseline)) {
          #calculate baseline (0 min) for each gene-species-replicate
          plot_data <- plot_data %>%
        ...
        #set expression label for hover text
        expr_label <- if (isTRUE(input$normalize_to_baseline)) "log2 FC" else "Expression"
        ...
        p <- ggplot(plot_data,
                    aes(x = Timepoint, y = Expression,
                        ...
                        text = paste("Gene:", Gene,
                                     ...
                                     "<br>Time:", Timepoint,
                                     paste0("<br>", expr_label, ": "), round(Expression, 2)))) +
          ...
          labs(
            y = if (isTRUE(input$normalize_to_baseline)) {
              "log2 Fold-Change (vs. 0 min)"
            } else {
              get_expression_label(input$global_transform)
            },
