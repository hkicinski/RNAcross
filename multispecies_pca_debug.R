...
common_hogs <- names(hog_gene_map)[sapply(hog_gene_map, function(hog) length(hog) >= 2)]

pdf("multispecies_pca_debug_variations.pdf", width = 12, height = 8)
...
for (input in inputs) {
  for (prep in pre_process) {
    for (arg_name in names(pca_args)) {
      ...
      sample_matrix <- sample_matrix[, na_proportion < 0.5, drop = FALSE]
